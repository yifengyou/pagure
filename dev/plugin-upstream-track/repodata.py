...
def do_main(pkg_info, count, debug):
    pkg_info['check'] = check_upstream_repo(repo_reflect(pkg_info['name']))
    if pkg_info['check'] == 'OK':
    ...
    if debug:
        ...
        print(
            "[%-4d] %-3s %-14s repo: %s" % (count, pkg_info['check'], pkg_info['created'], pkg_info['upstream'])
        )
...
def main(debug=True):
    ...
    if debug:
        print("total pkg num : %d" % len(pkgs_list))
    for (url, pkg) in pkgs_list:
        ...
        pool.apply_async(do_main, args=(pkg_info, count, debug))
        # result.append(ret)
